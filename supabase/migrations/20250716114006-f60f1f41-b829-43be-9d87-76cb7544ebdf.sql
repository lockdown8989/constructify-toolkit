...
BEGIN
    ...
    LOOP
        ...
        IF employee_record.is_scheduled AND employee_record.scheduled_start IS NOT NULL THEN
            ...
            IF current_time > (employee_record.scheduled_start + (grace_period_minutes || ' minutes')::interval) THEN
    ...
    LOOP
        ...
        IF employee_record.scheduled_end IS NOT NULL AND 
           current_time > (employee_record.scheduled_end + (grace_period_minutes || ' minutes')::interval) THEN
            
            -- Calculate overtime minutes
            UPDATE attendance 
            SET overtime_minutes = EXTRACT(EPOCH FROM (
                current_time - employee_record.scheduled_end
            ))/60,
