...
create policy if not exists "Users can view their own payroll settings"
  on public.payroll_settings for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own payroll settings"
  on public.payroll_settings for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own payroll settings"
