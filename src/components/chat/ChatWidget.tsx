...
export const ChatWidget = () => {
  ...
  useEffect(() => {
    ...
    const setupPresence = async () => {
      ...
      const {
        data: employee
      } = await supabase.from('employees').select('id, name, avatar_url').eq('user_id', user.id).single();
      if (!employee) {
    ...
    const presenceChannel = supabase.channel('user-presence-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'user_presence'
    }, payload => {
  ...
  const updateConnectedUsers = async () => {
    ...
    const {
      ...
      `).neq('user_id', user.id).not('user_id', 'is', null);
  ...
  useEffect(() => {
    if (!user) return;
    const channel = supabase.channel('chat-messages-notifications').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'messages'
    }, payload => {
  ...
  const renderChatModeSelection = () => <div className="p-6 md:p-6 space-y-6 md:space-y-4 safe-area-inset">
      <div className="flex items-center justify-between mb-8 md:mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setIsOnline(!isOnline)} className={cn("relative inline-flex items-center px-4 py-2 rounded-full transition-all duration-300 ease-in-out touch-manipulation min-w-[100px] h-10", isOnline ? "bg-green-500 text-white" : "bg-red-500 text-white")}>
              <span className={cn("absolute w-6 h-6 bg-white rounded-full transition-transform duration-300 ease-in-out shadow-sm", isOnline ? "transform translate-x-0" : "transform translate-x-8")} />
              <span className="text-sm font-medium relative z-10 mx-auto">
                {isOnline ? "Online" : "Offline"}
              </span>
            </button>
          </div>
          <Button variant="ghost" size="sm" onClick={() => setIsOpen(false)} className="p-3 h-auto touch-manipulation md:hidden rounded-full hover:bg-muted/50">
            <X className="w-6 h-6" />
          </Button>
        </div>
      </div>
      ...
      {/* AI Assistant option (restricted to admin and payroll) */}
      {(['admin','payroll'].includes(currentUserRole)) && (
  ...
  const renderUserSelection = () => <div className="p-6 md:p-4 h-full flex flex-col safe-area-inset">
      <div className="flex items-center justify-between mb-6 md:mb-4">
        <h2 className="font-bold text-xl md:text-base text-foreground">Select User to Chat</h2>
        <Button variant="ghost" size="sm" onClick={resetChat} className="p-3 h-auto touch-manipulation rounded-full hover:bg-muted/50">
          <X className="w-6 h-6 md:w-5 md:h-5" />
        </Button>
      </div>
      
      <div className="flex-1 space-y-2 overflow-y-auto">
        {connectedUsers.length === 0 ? <p className="text-xs md:text-sm text-muted-foreground text-center py-6 md:py-8">
            No users available to chat with
          </p> : connectedUsers.map(connectedUser => <Button key={connectedUser.id} onClick={async () => {
        ...
        const {
          data: employee
        } = await supabase.from('employees').select('id').eq('user_id', connectedUser.id).single();
      ...
      }} className="w-full p-4 md:p-4 h-auto justify-start bg-muted/50 hover:bg-muted text-left touch-manipulation min-h-[72px] rounded-xl active:scale-[0.98] transition-transform" variant="ghost">
              <div className="flex items-center gap-4 w-full">
                <div className="relative">
                  <Avatar className="w-12 h-12 md:w-10 md:h-10">
                    <AvatarImage src={connectedUser.avatar_url || ''} alt={connectedUser.name} />
                    <AvatarFallback className="bg-primary/10 text-primary font-semibold">
                      {connectedUser.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <Circle className={cn("absolute -bottom-1 -right-1 w-4 h-4 md:w-3 md:h-3 bg-background rounded-full border-2 border-background", connectedUser.isOnline ? "text-green-500 fill-current" : "text-gray-400 fill-current")} />
                </div>
  ...
  const renderChatInterface = () => <>
      <div className="p-4 md:p-4 bg-muted/50 border-b safe-area-inset-top">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={resetChat} className="p-2 h-auto touch-manipulation">
              <X className="w-5 h-5" />
            </Button>
            <h3 className="font-semibold text-lg md:text-base truncate">
              {chatMode === 'ai' ? 'AI Assistant' : selectedUser?.name || 'Chat'}
            </h3>
            {chatMode === 'human' && selectedUser && <Circle className={cn("w-3 h-3 md:w-3 md:h-3 ml-1 flex-shrink-0", selectedUser.isOnline ? "text-green-500 fill-current" : "text-gray-400")} />}
          </div>
          <Button variant="ghost" size="sm" onClick={() => setIsOpen(false)} className="p-2 h-auto touch-manipulation md:hidden">
            <X className="w-5 h-5" />
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 md:p-4 overflow-y-auto">
        {chatMessages.length === 0 ? <p className="text-xs md:text-sm text-muted-foreground">
            {chatMode === 'ai' ? "Hi! I'm your AI assistant. How can I help you today?" : `Start a conversation with ${selectedUser?.name}`}
          </p> : <div className="space-y-4 md:space-y-4">
            {chatMessages.map(msg => {
          // Determine if this message was sent by the current user
          const isCurrentUser = msg.sender_id === currentEmployee?.id;
          return <div key={msg.id} className={`flex ${msg.sender_type === 'ai_bot' ? 'justify-start' : isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] md:max-w-xs rounded-2xl p-4 md:p-4 ${msg.sender_type === 'ai_bot' ? 'bg-muted text-foreground' : isCurrentUser ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground'}`}>
                    <p className="text-sm md:text-sm leading-relaxed">{msg.content}</p>
                    <p className="text-xs opacity-70 mt-2 md:mt-2">
                      {new Date(msg.created_at).toLocaleTimeString()}
                    </p>
                    {msg.sender && msg.sender_type !== 'ai_bot' && <p className="text-xs opacity-60 mt-1">
                        {msg.sender.name}
                      </p>}
                  </div>
                </div>;
        })}
            {(chatLoading || isTyping) && <div className="flex justify-start">
                <div className="bg-muted rounded-lg p-3 md:p-4">
                  <div className="flex gap-1">
                    <div className="w-2 h-2 bg-muted-foreground/60 rounded-full animate-bounce"></div>
                    <div className="w-2 h-2 bg-muted-foreground/60 rounded-full animate-bounce" style={{
                animationDelay: '0.1s'
              }}></div>
                    <div className="w-2 h-2 bg-muted-foreground/60 rounded-full animate-bounce" style={{
                animationDelay: '0.2s'
      ...
      <div className="p-4 md:p-4 border-t bg-background safe-area-inset-bottom">
        <div className="flex gap-3">
          <input type="text" value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && (chatMode === 'ai' ? handleSendAiMessage() : handleSendHumanMessage())} placeholder="Type your message..." className="flex-1 px-4 py-4 md:py-3 text-base md:text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 touch-manipulation" disabled={chatLoading || isTyping} />
          <Button size="sm" onClick={chatMode === 'ai' ? handleSendAiMessage : handleSendHumanMessage} disabled={!message.trim() || chatLoading || isTyping || chatMode === 'human' && !currentChat} className="px-4 py-4 md:py-3 touch-manipulation rounded-xl min-w-[56px]">
  ...
  return <>
      {/* Chat Window */}
      {isOpen && <div className={cn("fixed bg-background border shadow-lg z-50 flex flex-col",
    // Desktop
    ...
    }} className={cn("fixed shadow-lg z-40 p-0 touch-manipulation", "bg-primary hover:bg-primary/90 text-primary-foreground", "transition-all duration-200 ease-in-out rounded-full",
    // Desktop
    "md:bottom-4 md:right-4 md:w-14 md:h-14",
    // Mobile - larger touch target, better positioning
    "max-md:bottom-6 max-md:right-6 max-md:w-16 max-md:h-16")} aria-label={isOpen ? "Close chat" : "Open chat"}>
        {isOpen ? <X className="w-6 h-6 md:w-6 md:h-6" /> : <>
            <MessageCircle className="w-6 h-6 md:w-6 md:h-6" />
            {unreadCount > 0 && <Badge variant="destructive" className="absolute -top-2 -right-2 w-6 h-6 rounded-full p-0 flex items-center justify-center text-xs font-bold">
