...
import { HeatmapToggle, HeatmapBar, ConflictDetails } from '@/components/schedule/conflict-heatmap';
import { useConflictHeatmap } from '@/hooks/use-conflict-heatmap';

interface WeeklyGridWithHeatmapProps {
  employees: Employee[];
  weekStats: WeekStats;
...
const WeeklyGridWithHeatmap: React.FC<WeeklyGridWithHeatmapProps> = ({
  employees,
  weekStats,
  openShifts,
  formatCurrency,
  ...
  const { isEnabled: isHeatmapEnabled } = useConflictHeatmap();
          ...
          <div className="divide-y divide-gray-100">
            {employees.map(employee => (
              <div key={employee.id} className="relative">
                ...
                {/* Heatmap Bar Overlay */}
                {isHeatmapEnabled && !isMobile && (
                  <div className="absolute bottom-0 left-0 right-0 grid grid-cols-8 pointer-events-none">
                    <div className="border-r border-gray-200" />
                    {days.map((day, index) => (
                      <div key={day} className="border-r border-gray-200 last:border-r-0 p-1">
                        <HeatmapBar
                          employeeId={employee.id}
                          startTime={new Date(weekStats.startDate.getTime() + index * 24 * 60 * 60 * 1000)}
                          endTime={new Date(weekStats.startDate.getTime() + (index + 1) * 24 * 60 * 60 * 1000)}
                          className="mb-1"
                        />
                      </div>
                    ))}
...
export default WeeklyGridWithHeatmap;
